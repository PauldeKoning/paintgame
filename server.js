...
io.on('connection', function (client) {
    client.on('client_create_game', function () {
        games.push(new Game());
        io.to(client.id).emit('server_join_game_link', {"status": "success", "link": "/game.html?id=" + serverid});
    ...
    client.on('client_join_game', function (data) {
        if (data !== "") {
            ...
            for (i = 0; i < games.length; i++) {
                if (games[i].id == data) {
                    io.to(client.id).emit('server_join_game_link', {
                        "status": "success",
                        "link": "/game.html?id=" + data
                    });
                    joined = 1;
                }
            }
            ...
            io.to(client.id).emit('server_join_game_link', {
                "status": "error",
                "message": "Input a number in the field!"
            });
        }
    });
            ...
            if (games[i].id == data.gameid) {
                for (j = 0; j < games[i].players.length; j++) {
                    if (games[i].players[j].clientid !== client.id) {
                        io.to(games[i].players[j].clientid).emit('server_new_dot', {
                            "color": data.color,
                            "x": data.x,
                            "y": data.y,
                            "size": data.size
            ...
            if (games[i].id == data.gameid) {
                for (j = 0; j < games[i].players.length; j++) {
                    if (games[i].players[j].clientid !== client.id && games[i].currentDrawer.clientid == client.id) {
                        io.to(games[i].players[j].clientid).emit('server_next_line_to', {
                            "color": data.color,
                            "x": data.x,
                            "y": data.y,
                            "size": data.size
    ...
    client.on('client_create_line', function (data) {
        for (i = 0; i < games.length; i++) {
            if (games[i].id == data.gameid) {
                for (j = 0; j < games[i].players.length; j++) {
                    if (games[i].players[j].clientid !== client.id && games[i].currentDrawer.clientid == client.id) {
                        io.to(games[i].players[j].clientid).emit('server_create_line', {
                            "color": data.color,
                            "x": data.x,
                            "y": data.y,
                            "size": data.size
    ...
    client.on('client_end_line', function (data) {
        for (i = 0; i < games.length; i++) {
            if (games[i].id == data.gameid) {
                for (j = 0; j < games[i].players.length; j++) {
                    if (games[i].players[j].clientid !== client.id && games[i].currentDrawer.clientid == client.id) {
                        io.to(games[i].players[j].clientid).emit('server_end_line', {
                            "color": data.color,
                            "x": data.x,
                            "y": data.y,
                            "size": data.size
                ...
                if (data.message !== "") {
                    if (data.message == games[i].currentWord) {
                        if (client.id !== games[i].currentDrawer.clientid) {
                            for (j = 0; j < games[i].players.length; j++) {
                                if (games[i].drawTimer <= 60 && games[i].players[j].hasGuessed == 0) {
                                    if(games[i].players[j].clientid == client.id ) {
                                        for(k = 0; k < games[i].players.length; k++) {
                                            io.to(games[i].players[k].clientid).emit('server_send_message', {
                                                "username": "SERVER",
                        ...
                        for (j = 0; j < games[i].players.length; j++) {
                            io.to(games[i].players[j].clientid).emit('server_send_message', {
                                "username": username,
    ...
    for (i = 0; i < games.length; i++) {
        ...
        if (games[i].drawTimer >= 60) {
            for (j = 0; j < games[i].players.length; j++) {
                ...
                if(65 - games[i].drawTimer > 0) {
                    io.to(games[i].players[j].clientid).emit('server_send_message', {
                        "username": "SERVER",
                        "message": "Next round in " + (65 - games[i].drawTimer) + " seconds"
                    });
                }
            }
        ...
        amountGuessed = 0;
        for (j = 0; j < games[i].players.length; j++) {
            if(games[i].players[j].hasGuessed == 1) {
                amountGuessed++;
            }
        }
        if(games[i].players.length > 1 && amountGuessed == games[i].players.length - 1) {
            games[i].drawTimer = 60;
            for (j = 0; j < games[i].players.length; j++) {
                io.to(games[i].players[j].clientid).emit('server_send_message', {
                    "username": "SERVER",
